...
class M2E_MultichannelConnect_Model_Magento_Tax_Helper
{
    public function hasRatesForCountry($countryId)
    {
        return Mage::getModel('tax/calculation_rate')
            ...
            ->addFieldToFilter(
                'code',
                array('neq' => M2E_MultichannelConnect_Model_Magento_Tax_Rule_Builder::TAX_RATE_CODE_PRODUCT)
            )
            ->addFieldToFilter(
                'code', array('neq' => M2E_MultichannelConnect_Model_Magento_Tax_Rule_Builder::TAX_RATE_CODE_SHIPPING)
            )
            ->getSize();
    }
